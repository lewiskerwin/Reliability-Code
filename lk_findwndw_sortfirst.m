        ...
        for isub = 1:size(sorted,4)
            ...
            for idist = 1:size(sorted,3)
                ...
                while 1
                    ...
                    targetdata = smooth(double(abs(mean(sorted(cfg.regs(ireg).chan,targettimeidx,idist,isub),1)')),5);
                ...
                bestloc = loc(bestidx);
                peakdata.amplat(ireg,iwndw,idist,isub) = bestloc-1 + (cfg.peak.target(iwndw)-cfg.peak.wiggle(iwndw));
                ampidx(ireg,iwndw,idist,isub) = bestloc-1 +  targettimeidx(1); %Add rank (in target range) to location of target-range
                %translate latency back into the full data spectrum
                peakrangeidx = ampidx(ireg,iwndw,idist,isub) - cfg.peak.width(iwndw):  ampidx(ireg,iwndw,idist,isub) + cfg.peak.width(iwndw);
                peakrangelat = cfg.alltimes(peakrangeidx);
                %
                
                %SAVE AMP OF THIS DATA POINT
                peakdata.ampmax(ireg,iwndw,idist,isub) = bestpeak;
                ...
                tmp = trapz(standardpeakrangeidx,mean(sorted(cfg.regs(ireg).chan,standardpeakrangeidx,idist,isub),1)); %Intgrate data at that index
                %tmp = tmp/(data(isub,idist).EEG.baseline_variance)^0.5; %An optional step here, where we normalize by dividing by standard deviation of baseline
                peakdata.ampsauc(ireg,iwndw,idist,isub) = tmp; %Name 'AUC' in data and equate it to the integral above
