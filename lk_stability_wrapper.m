...
switch cfg.ProjectName
    ...
    case 'rtms' %New folder within allresults
        cfg.file.subs = {'116'; '118'; '119';'120';'121';'122';'123';'124';'125';'126';'127';'128'}; cfg.file.preconds = {'1';'2'};%'final';'washout'};
        cfg.file.subprefix = 'rtms_';
        cfg.file.precondprefix = {'pre', ''; '_', '_'};
        cfg.file.precond_include = [];
        cfg.file.precond_exclude(1,:) = {'arm', 'PostRTMS','PreRTMS','fromConcat','bad'};
        cfg.file.day = {'tp1';'tp2'};
...
cfg.featurelabel = {'Peak Latency', 'Peak Amplitude', 'Area Under Curve (Centered Window)', 'Area Under Curve (Standard Window)'};
cfg.featureaxislabel = {'Peak Latency (ms)';'Peak Amplitude (uv)';'Area Under Curve (arb. units)';'Area Under Curve (arb. units)'};
...
if ~isempty(cfg.file.day) 
    cfg.comparison = {'alt','split','cond', 'day','TI'};
...
cfg.wndwnumber = 4;
cfg.alltimes
clear timeidx peakidxrange
xmin=0;
xmax=250;
timerange = xmin:xmax;
timeidx = find(data.times(:,1,1)==timerange(1)):find(data.times(:,1,1)==timerange(end));

alldata = mean(abs(mean(mean(mean(mean(data.amp(1,:,:,:,:,:),6),3),4),5)),1);
smoothed = smooth(alldata,10)
figure
plot(timerange,smoothed(timeidx));
[peakamp,peakloc] = findpeaks(double(smoothed))
%peakidxrange = find(cfg.alltimes>0 & find(cfg.alltimes<250))
peakidx = (find(peakloc >timeidx(1) & peakloc<timeidx(end)))
peakloc = peakloc(peakidx)
peakamp = peakamp(peakidx)
[peakamp,heightsortidx] = sort(peakamp,'descend');
peakloc = peakloc(heightsortidx);
cfg.peak.target = sort(cfg.alltimes(peakloc(1:cfg.wndwnumber))');
gavpeaks=  sort(cfg.alltimes(peakloc(1:cfg.wndwnumber))');
partition(2:cfg.wndwnumber) = (gavpeaks(1:cfg.wndwnumber-1)+gavpeaks(2:cfg.wndwnumber))./2
...
cfg.itnumber=4; %10 interations can increase to 100
cfg.bootlength = cfg.trialincr/2; %Makes 5 long for compatiblity with alt/split
cfg.sampleperboot = cfg.bootlength; %round(cfg.bootlength/2,0); % For each boot, take 50%

clear stats
tic
cfg.conditionforalt =1;
cfg.conditionforsplit = 1;
cfg.dayforcond = 1;
[cfg, stats ,data] = lk_halfsamplelimit(data,cfg);
toc



for ifeature=1:size(cfg.feature,2)
    [anovadata, stats] = lk_anova(data,stats,cfg,ifeature);
end
...
icond=1;
icomparison=1;
lk_waveformplot3(data,cfg,iTI,ireg,isub,icond,icomparison);


%PLOT EFFECT OF INCREASING TRIAL NUMBER ON CCC
ifeature = 4; %Adjust here
icomparison = 3; %and here
lk_plotregbystat(stats,cfg,ifeature,icomparison);

istat=5;
istat=3;
for ifeature=1:length(cfg.feature)
lk_plotregbycompwithpic(stats,cfg,ifeature,istat);
end

%PLOT EACH WNDW SEPARATELY
for iwndw=1:cfg.wndwnumber
    lk_plotregbycomp_onewndw(stats,cfg,ifeature,istat,iwndw);
end
...
%MOTOR THRESHOLD
[cfg] = lk_motorthreshold(cfg,data,iTI);
