...
fname = [whosedata '_reliability_by_TI_' Date]
...
featuretoplot = {'ampauc', 'amplat'};
comparisonlabel = {'Condition 1 vs 2', 'Split 1 vs 2', 'Odd vs Even Trials'};
comparisontoplot = {'cond', 'split', 'alt'};
feature = 1; %Adjust here
comparison=3; %and here
fctoplot = [featuretoplot{feature} comparisontoplot{comparison}];


stattoplot(:)= {'CCC','pearson','SDC'};
...
for iwndw=1:size(cfg.peak.target,2)
    for ireg = 1:size(cfg.regs,2)
        ...
        subplot(size(cfg.peak.target,2),width,(iwndw-1)*width+1)
        datatoplot = reliability.(fctoplot).(stattoplot{1});
    ...
    if ~comparisonlabeled 
        ...
        title(sprintf(TITLE,comparisonlabel{comparison},cfg.peak.wndwnames{iwndw}));
        comparisonlabeled=1;
    else
        TITLE = '%s-ms peak';
    ...
    xlabel('Trial Number'); ylabel('Concordance Correlation Coefficient');
    axis( [0 cfg.trialnumber 0 1]);
    box off; grid on;
    if ~Legend
...
%PLOT EFFECT OF INTREASING TRIAL NUMBER ON  Pearson

comparisonlabeled=0;
for iwndw=1:size(cfg.peak.target,2)
    for ireg = 1:size(cfg.regs,2)
        subplot(size(cfg.peak.target,2),width,(iwndw-1)*width+2)
        plot(cfg.trialincr:cfg.trialincr:cfg.trialnumber,squeeze(reliability.(fctoplot).(stattoplot{2}(:,iwndw,:))),'-o');
    end
    ...
    if ~comparisonlabeled 
        ...
        xlabel('Trial Number'); ylabel('Pearson Coefficient');
        axis( [0 cfg.trialnumber 0 1]);
        box off; legend boxoff; grid on;
...
for iwndw=1:size(cfg.peak.target,2)
    for ireg = 1:size(cfg.regs,2)
        subplot(size(cfg.peak.target,2),width,(iwndw-1)*width+3)
        plot(cfg.trialincr:cfg.trialincr:cfg.trialnumber,squeeze(reliability.(fctoplot).(stattoplot{3}(:,iwndw,:))),'-o');
      ...
      %Add "odd vs even" only on top row of graphs
   if ~comparisonlabeled 
        TITLE = 'Smallest Detectable Change \n %s \n %s-ms peak';
        title(sprintf(TITLE,comparisonlabel{comparison},cfg.peak.wndwnames{iwndw}));
        comparisonlabeled=1;
    else
        TITLE = '%s-ms peak';
     ...
     xlabel('Trial Number'); ylabel('Smallest Detectable Change');
        axis( [0 cfg.trialnumber 0 200]);
        box off; legend boxoff;grid on;
