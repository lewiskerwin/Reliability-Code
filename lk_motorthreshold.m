function [cfg] = lk_motorthreshold(cfg,data,iTI)
...
for ifeature = 1:width 
    for iwndw = 1:height
        ...
        MTquant=squeeze(mean(data.(cfg.feature{ifeature}).day.mean(:,iwndw,:,:,iTI),1))';%day so as to get whole day
        MTquantconcat = reshape(MTquant,[],1); %Note day 1 and 2 are separated here
        ...
        switch iwndw
            case 1 %top
                if ifeature == 2 %middle
                    TITLE = 'Peak Feature as a Function of Motor Threshold \n %s ';
...
fname = [cfg.ProjectName '_Feature=F(MT)_based_on_first_' num2str(iTI*cfg.trialincr) '_trials' Date];
