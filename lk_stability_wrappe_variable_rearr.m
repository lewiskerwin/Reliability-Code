    ...
    for ireg =1:cfg.regnumber
          for iwndw = 1:cfg.wndwnumber    
              ...
              for iboot= 1:cfg.bootnumber
                  bootrange = (iboot-1)*cfg.bootlength+1:cfg.bootlength*iboot;
                  prebootdata(iboot,:,:) = squeeze(mean(reliability.ampauc(ireg,iwndw,bootrange,:,:,iTI),3));
              end
              ...
              for iit=1:cfg.itnumber %100 itereations
                  ...
                  statmat = squeeze(mean(prebootdata(bootsignature(:,iit),:,:),1))';
                 [pearson(iit), ttest(iit), CCC(iit), ICC(iit), SDC(iit)] = lk_stats(statmat,cfg);
          
              end
              reliability.ampauccond.pearson(ireg,iwndw,iTI) = mean(pearson);
              reliability.ampauccond.pearsons(ireg,iwndw,iTI) = std(pearson);
              reliability.ampauccond.ttest(ireg,iwndw,iTI) = mean(ttest);
              reliability.ampauccond.ttests(ireg,iwndw,iTI) = std(ttest);
              reliability.ampauccond.CCC(ireg,iwndw,iTI) = mean(CCC);
              reliability.ampauccond.CCCs(ireg,iwndw,iTI) = std(CCC);
              reliability.ampauccond.ICC(ireg,iwndw,iTI) = mean(ICC);
              reliability.ampauccond.ICCs(ireg,iwndw,iTI) = std(ICC);
              reliability.ampauccond.SDC(ireg,iwndw,iTI) = mean(SDC);
              reliability.ampauccond.SDCs(ireg,iwndw,iTI) = std(SDC);
              reliability.ampauccond.SDCp(ireg,iwndw,iTI) = reliability.ampauccond.SDC(ireg,iwndw,iTI)./mean(mean(mean(prebootdata)));

             
          end
    end
    
    %END OF BOOTSTRAPPING (the stuff below is commented out b/c
    %bootstrapping does it already)
%     
%     %LUMP BY CONDITION
%     reliability.ampauccond.data(:,:,:,:,:,iTI) = mean(reliability.ampauc(:,:,1:iTI*cfg.trialincr,:,:,iTI),3)
%     
%     %regx wndw x 1split x cond x sub x iTI
%     for ireg =1:cfg.regnumber
%         for iwndw = 1:cfg.wndwnumber
%             statmat = squeeze(reliability.ampauccond.data(ireg,iwndw,:,:,:,iTI))';
%             [pearson, ttest, CCC, ICC, SDC] = lk_stats(statmat,cfg);
%             reliability.ampauccond.pearson(ireg,iwndw,iTI) = pearson;
%             reliability.ampauccond.ttest(ireg,iwndw,iTI) = ttest;
%             reliability.ampauccond.CCC(ireg,iwndw,iTI) = CCC;
%             reliability.ampauccond.ICC(ireg,iwndw,iTI) = ICC;
%             reliability.ampauccond.SDC(ireg,iwndw,iTI) = SDC;
%             
%         end
%     end
