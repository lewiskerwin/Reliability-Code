...
for iwndw=1:size(cfg.peak.target,2)
    ...
    if iwndw==1
        ...
        if icomparison == (floor(width/2)+1)
            TITLE = '%s of %s \n %s \n %s-ms peak';
            title(sprintf(TITLE,statlabel{istat},featurelabel{ifeature},comparisonlabel{icomparison},cfg.peak.wndwnames{iwndw}));
        else
            TITLE = '%s \n %s-ms peak';
            title(sprintf(TITLE,comparisonlabel{icomparison},cfg.peak.wndwnames{iwndw}));
        end
    else
        TITLE = '%s-ms peak';
...
fname = [cfg.ProjectName '_' cfg.feature{ifeature} '_' stattoplot{istat} '_' [cfg.comparison{:}] '_' Date];
