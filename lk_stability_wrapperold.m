...
%raw data)
wndw = [0 50;50 100;100 150;150 250];
wndwNames = {'0-50';'50-100';'100-150';'150-250'};
...
for ireg = 1:size(reliability.AUC,3)
    for iwndw = 1:size(reliability.AUC,4)
        ...
        miniAUC = squeeze(reliability.AUC(:,:,ireg,iwndw,:)); %Let's look only at non-one dimensions
        ...
        for isub=1:size(miniAUC,1)
            for iconds=1:size(miniAUC,2)
                for isplit=1:size(miniAUC,3)
                    ...
                    tdAUC(1,cnt) = miniAUC(isub,iconds,isplit);
        ...
        for idim=1:ndims(miniAUC)+1;
            ...
            reliability.stats(ireg,iwndw).VAR(idim) = var(idim);
        end
        %Then Calculate SEM, SDC and ICC!
        for idim=1:ndims(miniAUC);
            reliability.stats(ireg,iwndw).SEM(idim) = (var(idim)+var(ndims(miniAUC)+1))^0.5;
            reliability.stats(ireg,iwndw).SDC(idim) = reliability.stats(ireg,iwndw).SEM(idim) * 2^0.5 * 1.96;
            reliability.stats(ireg,iwndw).ICC(idim) = var(idim)/sum(var(idim:length(var)));
            
            disp([ 'In the ' cfg.regs(ireg).name ' region and window ' wndwNames{iwndw} ' the SEM across ' tbl{idim+1,1}  ' is ' num2str(reliability.stats(ireg,iwndw).SEM(idim))]);
            disp([ 'In the ' cfg.regs(ireg).name ' region and window ' wndwNames{iwndw} ' the ICC across ' tbl{idim+1,1} ' is ' num2str(reliability.stats(ireg,iwndw).ICC(idim))]);
